...
// Mock EmbeddingService for the script
const EmbeddingService = {
    generate: async (text: string) => {
        // Generate a dummy vector of 1536 dimensions (OpenAI ada-002 size)
...
async function main() {
    ...
    for (const doc of documents) {
        ...
        if (docError) {
            console.error('Error inserting doc:', docError);
            continue;
        }
        ...
        // Insert chunk
        const { error: chunkError } = await supabase.from('legal_chunks').insert([{
            document_id: docId,
    ...
    console.log(`Successfully seeded ${processed} legal documents into RAG.`);
}
