...
export default function AdminValidacionPage() {
    ...
    const [selectedMissionId, setSelectedMissionId] = useState<string | null>('1');

    const handleAprobar = () => {
        if (!selectedMissionId) return;
        alert('Misión aprobada y publicada con éxito.');
        setPendingMissions(prev => prev.filter(m => m.id !== selectedMissionId));
        setSelectedMissionId(null);
    };

    const handleRechazar = () => {
        if (!selectedMissionId) return;
        alert('Misión rechazada. Se enviará feedback al sistema IA.');
        setPendingMissions(prev => prev.filter(m => m.id !== selectedMissionId));
        setSelectedMissionId(null);
    };

    const selectedMission = pendingMissions.find(m => m.id === selectedMissionId);
                ...
                <div className="card" style={{ flex: '0.35', display: 'flex', flexDirection: 'column', padding: '1.5rem' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', overflowY: 'auto' }}>
                        {pendingMissions.length === 0 ? (
                            <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '2rem 0' }}>No hay tareas pendientes.</div>
                        ) : (
                            pendingMissions.map((item) => (
                                <div
                                    key={item.id}
                                    ...
                                    style={{
                                        padding: '1rem',
                                        borderRadius: '8px',
                                        background: selectedMissionId === item.id ? 'rgba(201, 162, 39, 0.1)' : 'rgba(0,0,0,0.2)',
                                        border: selectedMissionId === item.id ? '1px solid var(--primary)' : '1px solid transparent',
                                        cursor: 'pointer'
                                    }}
                                >
                    ...
                    {selectedMission ? (
                        <>
                            <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--border-color)', background: 'rgba(0,0,0,0.2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div>
                                    <span className="badge" style={{ marginBottom: '0.5rem' }}>{selectedMission.tipo}</span>
                                    <h2 style={{ fontSize: '1.5rem', margin: 0 }}>{selectedMission.titulo}</h2>
                                </div>
                                <div style={{ display: 'flex', gap: '0.5rem' }}>
                                    <button className="btn btn-danger" style={{ padding: '0.5rem 1rem' }} onClick={handleRechazar}>Rechazar</button>
                                    <button className="btn btn-secondary" style={{ padding: '0.5rem 1rem' }}>Editar</button>
                                    <button className="btn btn-success" style={{ padding: '0.5rem 1.5rem' }} onClick={handleAprobar}>Aprobar y Publicar</button>
                                </div>
                            </div>
                                ...
                                <div style={{ background: 'rgba(245, 158, 11, 0.1)', padding: '1rem', borderRadius: '8px', border: '1px dashed var(--warning)', display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                                    ...
                                    <div>
                                        ...
                                        <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: '0.9rem', lineHeight: '1.5' }}>
                                            He generado esta misión basándome en el "Estatuto de los Trabajadores". La confianza en la extracción legal es del {selectedMission.ia_confidence}%. Por favor, verifica que la rúbrica de evaluación es correcta antes de aprobar.
                                        </p>
                                    </div>
                                </div>
                        ...
                        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)' }}>
                            Selecciona un elemento de la lista para revisarlo.
                        </div>
                    )}
                </div>
            </div>
